//===----------------------------------------------------------------------===//
// Parser Implementation - Kaleidoscope Parser
...
void Parser::InstallStandardOperators() {
...
std::unique_ptr<ExprAST> Parser::ParseVarExpr() {
    ...
    while (true) {
        std::string Name = TheLexer.getIdentifierStr();
        getNextToken(); // eat identifier.
...
std::unique_ptr<PrototypeAST> Parser::ParsePrototype() {
    ...
    case tok_binary:
        ...
        if (CurTok == tok_number) {
            if (TheLexer.getNumVal() < 1 || TheLexer.getNumVal() > 100)
                return LogErrorP("Invalid precedence: must be 1..100");
            BinaryPrecedence = (unsigned)TheLexer.getNumVal();
            getNextToken();
        }
        break;
    }
